...
class Event(object):
    ...
    def get_pred_index(self, pred2idx):
        return (
            pred2idx[self.pred]
            if self.pred in pred2idx
            else pred2idx[indices.PRED_OOV]
        )
    ...
    def get_arg_indices(self, argn, argw2idx, use_head=False, arg_len=-1):
        ...
        ret = [
            argw2idx[tok]
            if tok in argw2idx
            else argw2idx[indices.UNKNOWN_ARG_WORD]
            for tok in sp
        ]
        # padding 0
        if len(ret) < arg_len:
...
class EventRelation:
    ...
    def __repr__(self):
        return "{} ||| {} ||| {}".format(self.rtype_idx, self.e1, self.e2)
    ...
    def to_indices(self, pred2idx, argw2idx, use_head=False, arg_len=-1):
        return [
            ...
            self.e1.get_arg_indices(
                0, argw2idx, use_head=use_head, arg_len=arg_len
            ),
            self.e1.get_arg_indices(
                1, argw2idx, use_head=use_head, arg_len=arg_len
            ),
            self.e1.get_arg_indices(
                2, argw2idx, use_head=use_head, arg_len=arg_len
            ),
            self.e2.get_pred_index(pred2idx),
            self.e2.get_arg_indices(
                0, argw2idx, use_head=use_head, arg_len=arg_len
            ),
            self.e2.get_arg_indices(
                1, argw2idx, use_head=use_head, arg_len=arg_len
