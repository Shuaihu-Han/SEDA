...
class ConfusionMatrix(object):
    ...
    def __init__(self, alphabet=None):
        if alphabet is None:
            ...
            self.matrix = numpy.zeros(
                (self.INIT_NUM_CLASSES, self.INIT_NUM_CLASSES)
            )
        else:
            self.alphabet = alphabet
    ...
    def compute_micro_average_f1(self):
        ...
        total_predicted = numpy.sum(
            [
                x
                for i, x in enumerate(self.matrix.sum(1))
        ...
        total_gold = numpy.sum(
            [
                x
                for i, x in enumerate(self.matrix.sum(0))
                if negative_index == -1 or i != negative_index
            ]
        )
    ...
    def print_matrix(self):
        ...
        header = [" "] + [
            self.alphabet.get_label(i) for i in range(num_classes)
        ]
        rows = []
        # putting labels to the first column of rhw matrix
        for i in range(num_classes):
            row = [self.alphabet.get_label(i)] + [
                str(self.matrix[i, j]) for j in range(num_classes)
            ]
            rows.append(row)
    ...
    def print_summary(self):
        ...
        for i in range(self.alphabet.size()):
            ...
            if label != self.NEGATIVE_CLASS:
                ...
                lines.append(
                    "%s%s precision %1.4f\trecall %1.4f\tF1 %1.4f"
...
class Alphabet(object):
    ...
    def __eq__(self, other):
        return (
            self._index_to_label == other._index_to_label
    ...
    def get_label(self, index):
        ...
        if index >= self.num_labels:
            raise KeyError(
                "There are %d labels but the index is %d"
    ...
    def to_dict(self):
        return {"_label_to_index": self._label_to_index}
