...
def main():
    ...
    for i_batch in range(n_batches):
        ...
        e2idx, embeddings = build_embeddings(
            model, batch_questions, config, pred2idx, argw2idx, rtype
        )
        for q in batch_questions:
            pred = intrinsic.predict_mcnc(
                model, q, e2idx, embeddings, rtype, args.device
            )
            if pred == q.ans_idx:
                n_correct += 1
            else:
